#!/usr/bin/env bash
{
    ...
    while getopts "c:d:hv:ub" OPTION
    do
        case $OPTION in
            v)
                AVI_VERSION="$OPTARG"
    ...
    if [ $RUN_IN_BACKGROUND ]; then
        docker run -td --hostname $CONTAINER_HOSTNAME --name $CONTAINER_HOSTNAME -w /opt/avi -v $DIR:/opt/avi "$AVITOOLS_DOCKER_IMAGE" $CMD
        exit 0
    else
        docker run --rm -w /opt/avi -v $DIR:/opt/avi "$AVITOOLS_DOCKER_IMAGE" $CMD
        exit 0
    fi
}