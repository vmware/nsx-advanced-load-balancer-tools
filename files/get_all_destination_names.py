...
parser = argparse.ArgumentParser(
    description="AVI SDK based Script to get all destination"
)
...
parser.add_argument(
    "-c", "--controller", required=True, help="Source Controller IP address"
)
parser.add_argument(
    "-a", "--api_version", required=False, default=None, help="Api Version"
)
...
if args.api_version:
    ...
    session = ApiSession.get_session(
        args.controller, args.username, args.password, max_api_retries=3
    )
    api_version = session.remote_api_version["Version"]
...
def check_se_in_provider_context(avi_session):
    ...
    if resp == None or resp.status_code != 200:
        print(
            f"Failed to get system configuration. Please validate your connection or the status of your Avi controller!"
        )
        sys.exit(1)
    else:
        resp = resp.json()
...
def get_cloud_list(avi_session):
    ...
    while page_end is not True:
        resp = avi_session.get(
            f"cloud?fields=name&page_size=200&page={page}", tenant="*"
        )

        if resp == None or resp.status_code != 200:
            print(
                f"Failed to get cloud. Please validate your connection or the status of your Avi controller!"
            )
            sys.exit(1)
        else:
            resp = resp.json()
...
def get_tenant_list(avi_session):
    ...
    while page_end is not True:
        resp = avi_session.get(
            f"tenant?fields=name&page_size=200&page={page}", tenant="*"
        )

        if resp == None or resp.status_code != 200:
            print(
                "Failed to collect tenant list. Please validate your connection or the status of your Avi controller!"
            )
            sys.exit(1)
        else:
            resp = resp.json()
...
def get_cloud_vrf(avi_session, cloud):
    ...
    while page_end is not True:
        resp = avi_session.get(
            f"vrfcontext?include_name=true&fields=name,tenant_ref&cloud_ref.name={cloud}&page_size=200&page={page}",
            tenant="*",
        )

        if resp == None or resp.status_code != 200:
            print(
                f"Failed to get cloud VRF. Please validate your connection or the status of your Avi controller!"
            )
            sys.exit(1)
        else:
            resp = resp.json()
...
def get_cloud_seg(avi_session, cloud):
    ...
    while page_end is not True:
        resp = avi_session.get(
            f"serviceenginegroup?include_name=true&fields=name,tenant_ref&cloud_ref.name={cloud}&page_size=200&page={page}",
            tenant="*",
        )

        if resp == None or resp.status_code != 200:
            print(
                f"Failed to get cloud SEG. Please validate your connection or the status of your Avi controller!"
            )
            sys.exit(1)
        else:
            resp = resp.json()
