...
export class StartMigrationWizardComponent {
  ...
  constructor(
    ...
    const groupMembers = {
      f5_ssh_user: ['', [Validators.required, Validators.email]],
    ...
    this.labControllerForm = this.formBuilder.group({
      avi_lab_user: ['', [Validators.required, Validators.email]],
  ...
  submit() {
    this.wizard.forceNext();
    const payload = {...this.destinationCtrlForm.value, ...this.certsAndKeysForm.value, ...this.labControllerForm.value, ...this.mapDestinationForm.value, avi_destination_version: 1 }
